...
template <class T>
constexpr vector_type<T> direction(const line_type<T> &l) { return vector_type(-l.b, l.a); }
template <class T>
constexpr vector_type<T> perpend(const line_type<T> &l, const vector_type<T> &v) {
  auto u = vector_type(l.a, l.b);
  auto d = -l.c / norm(u);
...
constexpr T dist(const line_type<T> &l, const vector_type<T> &v) { 
  return std::abs(l.a * v.x + l.b * v.y + l.c) / abs(vector_type(l.a, l.b)); 
}
